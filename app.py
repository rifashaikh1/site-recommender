...
st.set_page_config(page_title="Website Suggester", layout="centered")
st.title("Website Suggester 🔍")

# Input URL
user_url = st.text_input("Enter website URL (e.g., example.com)")

# Optional category input
user_category = st.selectbox(
    "Select category (optional, leave blank to auto-detect)",
    [""] + [
        "Adult","Business/Corporate","Computers and Technology",
...
if st.button("Find Similar Websites"):
    if not user_url:
        ...
        with st.spinner("Searching..."):
            results = find_similar_websites(user_url, user_category=user_category)
